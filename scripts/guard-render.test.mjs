...
/**
 * Liste les lignes de commande visibles, avec LA MEME sonde que
 * guard-render.mjs. Le test interrogeait « ps -eo args= » en dur : sous Windows
 * ce ps existe (MSYS) mais ignore « -eo », si bien que la sonde rendait du vide
 * et que les assertions « un rendu tourne » ne pouvaient jamais etre vraies.
 * Un test qui n'interroge pas le systeme comme le fait le code teste ne teste
 * pas le code teste (decision 020).
 */
function lignesDeCommande() {
  const sonde =
    process.platform === "win32"
      ...
      : ["ps", ["-eo", "args="]];
  const r = spawnSync(sonde[0], sonde[1], { encoding: "utf8" });
  return r.stdout || "";
}
...
async function lanceFauxRendu() {
  ...
  // La sonde ne voit le processus qu'une fois exec() termine.
  for (let i = 0; i < 40; i++) {
    if (/hyperframes render/.test(lignesDeCommande())) return;
    await new Promise((r) => setTimeout(r, 50));
  }
  throw new Error("le faux rendu n'apparait pas dans la sonde de processus");
}
