...
$DRY $SSH "mkdir -p ~/sauvegardes/$TS && \
  ...
  cp -r ~/.hermes/skills/video ~/sauvegardes/$TS/skills-video 2>/dev/null; \
  cp -r ~/.hermes/agent-hooks ~/sauvegardes/$TS/ 2>/dev/null; \
  cp ~/.hermes/shell-hooks-allowlist.json ~/sauvegardes/$TS/ 2>/dev/null; true"
echo "   ~/sauvegardes/$TS"
...
echo "── 6. adaptateur de hook Hermes ──"
# Hermes n'abandonne PAS sur un code de sortie non nul : « non-zero exit codes
# log a warning but never abort the agent loop ». Cet adaptateur traduit le
# refus de guard-portail.mjs en {"decision":"block"}, seule forme qu'Hermes
# honore. Il a vecu jusqu'au 04/08/2026 uniquement sur le VPS : hors depot,
# hors sauvegarde, hors test. Reconstruire la machine aurait rendu le verrou
# du portail decoratif, en silence.
$DRY scp -q -i "$KEY" scripts/hook-portail.py "$VPS:~/.hermes/agent-hooks/portail.py"
...
echo
echo "── 8. VÉRIFICATION sur le VPS ──"
$SSH 'cd ~/imcp
ok=1
...
[ -s ~/.hermes/agent-hooks/portail.py ] \
  && echo "  ✅ adaptateur de hook portail présent" \
  || { echo "  ❌ adaptateur de hook portail MANQUANT — le verrou serait décoratif"; ok=0; }
...
echo "  — hooks de verrouillage —"
chmod +x scripts/hook-budget.py ~/.hermes/agent-hooks/portail.py
# L allowlist controle la derive de mtime : redeployer un hook le DESACTIVE.
# Les DEUX hooks passent par ici — en oublier un le rend muet sans rien casser
# de visible, ce qui est le pire des modes de panne pour un verrou.
python3 - <<PYEOF
import json, os, datetime
A=os.path.expanduser("~/.hermes/shell-hooks-allowlist.json")
HOOKS=[os.path.expanduser(p) for p in
       ("~/imcp/scripts/hook-budget.py", "~/.hermes/agent-hooks/portail.py")]
def iso(t): return datetime.datetime.fromtimestamp(t, datetime.timezone.utc).isoformat().replace("+00:00","Z")
d=json.load(open(A))
d["approvals"]=[a for a in d["approvals"] if a["command"] not in HOOKS]
maintenant=iso(datetime.datetime.now(datetime.timezone.utc).timestamp())
for S in HOOKS:
    d["approvals"].append({"approved_at":maintenant,"command":S,"event":"pre_tool_call",
      "script_mtime_at_approval":iso(os.path.getmtime(S))})
